...
import { assertBlogPermission, invalidateBlogCache, auditBlog } from "./_helpers";

const bulkInput = z.object({ ids: z.array(z.uuid()).min(1).max(100), operation: z.enum(["publish", "archive", "restore", "delete"]) });
...
export const bulkBlogPostLifecycle = defineAction({
  ...
  handler: async (input, context) => {
    const user = await assertBlogPermission(context, { blog: [permissionFor[input.operation]] });
    const db = getDrizzle();
    const rows = await db.select().from(blogPosts).where(inArray(blogPosts.id, input.ids));
    if (rows.length !== input.ids.length) throw new ActionError({ code: "FORBIDDEN", message: "Un ou plusieurs articles ne sont pas accessibles." });
    ...
    for (const row of rows) auditBlog(context, user.id, auditFor[input.operation], { resource: "blog_posts", resourceId: row.id, metadata: { bulk: true, from: row.status, to } });
    invalidateBlogCache();

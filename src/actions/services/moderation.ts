...
import { assertServiceExists, assertServicePermission } from "./_helpers";
...
export const moderateServiceComment = defineAction({ input: z.object({ id: z.uuid(), serviceId: z.uuid(), status: z.enum(["APPROVED", "REJECTED", "SPAM", "TRASH"]) }), handler: async (input, context) => {
  const user = await assertServicePermission(context, { serviceComment: ["moderate"] }); await assertServiceExists(input.serviceId); const t = getServiceTranslations(requestLocale(context.request.headers));
...
export const moderateServiceReview = defineAction({ input: z.object({ id: z.uuid(), serviceId: z.uuid(), status: z.enum(["APPROVED", "REJECTED", "SPAM"]) }), handler: async (input, context) => {
  const user = await assertServicePermission(context, { serviceReview: ["moderate"] }); const service = await assertServiceExists(input.serviceId); const locale = requestLocale(context.request.headers); const t = getServiceTranslations(locale);
  ...
  auditService(context, user.id, "SERVICE_REVIEW_MODERATE", { resource: "serviceReviews", resourceId: input.id, metadata: { status: input.status, ratingCount, ratingAverage100, serviceProviderId: service.providerId } }); invalidateServicesCache();
  return { success: true, ratingCount, ratingAverage100 };
} });

export const resolveServiceReport = defineAction({ input: z.object({ id: z.uuid(), serviceId: z.uuid(), status: z.enum(["REVIEWED", "RESOLVED", "REJECTED"]) }), handler: async (input, context) => {
  const user = await assertServicePermission(context, { service: ["moderate"] }); await assertServiceExists(input.serviceId); const t = getServiceTranslations(requestLocale(context.request.headers));

...
export const createServiceAvailability = defineAction({ input: serviceAvailabilitySchema, handler: async (input, context) => {
  const user = await assertServicePermission(context, { service: ["update"] }); await assertServiceExists(input.serviceId); await assertServiceLockOwner(input.serviceId, user.id, context.locals.session?.id); await assertNoAvailabilityOverlap(input);
...
export const deleteServiceAvailability = defineAction({ input: z.object({ id: z.uuid(), serviceId: z.uuid() }), handler: async (input, context) => {
  const user = await assertServicePermission(context, { service: ["update"] }); await assertServiceExists(input.serviceId); await assertServiceLockOwner(input.serviceId, user.id, context.locals.session?.id); const result = await getDrizzle().delete(serviceAvailability).where(and(eq(serviceAvailability.id, input.id), eq(serviceAvailability.serviceId, input.serviceId))).returning({ id: serviceAvailability.id }); if (!result[0]) throw new ActionError({ code: "NOT_FOUND", message: "Disponibilité introuvable." }); auditService(context, user.id, "SERVICE_UPDATE", { resource: "serviceAvailability", resourceId: input.id, metadata: { serviceId: input.serviceId, action: "DELETE" } }); invalidateServicesCache(); return { success: true };
} });

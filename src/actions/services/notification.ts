...
import { assertServicePermission } from "./_helpers";
...
export const listServiceNotifications = defineAction({ input: z.object({ limit: z.coerce.number().int().min(1).max(100).default(50) }), handler: async (input, context) => { const user = await assertServicePermission(context, { service: ["read"] }); return getDrizzle().select({ notification: serviceNotifications }).from(serviceNotifications).where(and(eq(serviceNotifications.recipientId, user.id), isNull(serviceNotifications.readAt))).orderBy(desc(serviceNotifications.createdAt)).limit(input.limit); } });
export const markServiceNotificationRead = defineAction({ input: z.object({ id: z.uuid() }), handler: async (input, context) => { const user = await assertServicePermission(context, { service: ["read"] }); const db = getDrizzle(); const [notification] = await db.select({ id: serviceNotifications.id }).from(serviceNotifications).where(and(eq(serviceNotifications.id, input.id), eq(serviceNotifications.recipientId, user.id))).limit(1); if (!notification) return { success: false }; await db.update(serviceNotifications).set({ readAt: new Date() }).where(eq(serviceNotifications.id, notification.id)); return { success: true }; } });
export const markAllServiceNotificationsRead = defineAction({ input: z.object({}), handler: async (input, context) => { const user = await assertServicePermission(context, { service: ["read"] }); const db = getDrizzle(); const rows = await db.select({ id: serviceNotifications.id }).from(serviceNotifications).where(and(eq(serviceNotifications.recipientId, user.id), isNull(serviceNotifications.readAt))); if (rows.length) await db.update(serviceNotifications).set({ readAt: new Date() }).where(inArray(serviceNotifications.id, rows.map((row) => row.id))); return { success: true }; } });

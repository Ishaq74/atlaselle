...
async function assertAttributeDefinitionExists(definitionId: string) {
  ...
  if (!definition) throw new ActionError({ code: "NOT_FOUND", message: "Attribut introuvable." });
  return definition;
}
...
export const createServiceAttributeDefinition = defineAction({
  ...
  handler: async (input, context) => {
    const user = await assertServicePermission(context, { service: ["update"] }); const db = getDrizzle();
    const existing = await db.select({ id: serviceAttributeDefinitions.id }).from(serviceAttributeDefinitions).where(eq(serviceAttributeDefinitions.key, input.key)).limit(1);
    if (existing.length) throw new ActionError({ code: "CONFLICT", message: "Cette clé d'attribut existe déjà." });
    const [row] = await db.insert(serviceAttributeDefinitions).values({ key: input.key, label: input.label, type: input.type, options: input.options.length ? JSON.stringify(input.options) : null, required: input.required, sortOrder: input.sortOrder }).returning();
    auditService(context, user.id, "SERVICE_UPDATE", { resource: "serviceAttributeDefinitions", resourceId: row.id, metadata: { action: "CREATE" } }); invalidateServicesCache(); return row;
  },
});

export const setServiceAttributeValue = defineAction({
  input: z.object({ serviceId: z.uuid(), definitionId: z.uuid(), stringValue: z.string().max(5000).optional().nullable(), numberValue: z.coerce.number().int().optional().nullable(), booleanValue: z.boolean().optional().nullable(), selectedValue: z.string().max(5000).optional().nullable() }),
  handler: async (input, context) => {
    const user = await assertServicePermission(context, { service: ["update"] }); await assertServiceExists(input.serviceId); await assertServiceLockOwner(input.serviceId, user.id, context.locals.session?.id); const definition = await assertAttributeDefinitionExists(input.definitionId);
    ...
    auditService(context, user.id, "SERVICE_UPDATE", { resource: "serviceAttributeValues", resourceId: input.serviceId, metadata: { definitionId: input.definitionId } }); invalidateServicesCache(); return { success: true };
  },
});

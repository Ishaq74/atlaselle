...
export async function getBlogAdminPostData(locale: Locale, filters: BlogAdminPostFilters = {}) {
  const db = getDrizzle();
  const displayLocale = filters.translationLocale ?? locale;
  const page = Math.max(1, filters.page ?? 1);
  const limit = Math.min(100, Math.max(1, filters.limit ?? 20));
  const offset = (page - 1) * limit;
  const conditions = [eq(blogPostTranslations.locale, displayLocale)];
  ...
  if (filters.categoryId) {
    conditions.push(inArray(blogPosts.id, db.select({ postId: blogPostCategories.postId }).from(blogPostCategories).innerJoin(blogCategories, eq(blogCategories.id, blogPostCategories.categoryId)).where(eq(blogPostCategories.categoryId, filters.categoryId))));
  }
  if (filters.tagId) {
    conditions.push(inArray(blogPosts.id, db.select({ postId: blogPostTags.postId }).from(blogPostTags).innerJoin(blogTags, eq(blogTags.id, blogPostTags.tagId)).where(eq(blogPostTags.tagId, filters.tagId))));
  }
  ...
  const rows = await db
    ...
    .from(blogPosts)
    .innerJoin(blogPostTranslations, and(eq(blogPostTranslations.postId, blogPosts.id), eq(blogPostTranslations.locale, displayLocale)))
  ...
  const [{ total }] = await db.select({ total: count() }).from(blogPosts).innerJoin(blogPostTranslations, and(eq(blogPostTranslations.postId, blogPosts.id), eq(blogPostTranslations.locale, displayLocale))).where(and(...conditions));
  ...
  const [{ total: aggregateTotal }] = await db.select({ total: count() }).from(blogPosts);
  const statusRows = await db.select({ status: blogPosts.status, count: count() }).from(blogPosts).groupBy(blogPosts.status);
  const [{ views }] = await db.select({ views: sql<number>`coalesce(sum(${blogPosts.viewCount}), 0)` }).from(blogPosts);
  const [{ featured }] = await db.select({ featured: count() }).from(blogPosts).where(eq(blogPosts.isFeatured, true));
  const [{ sticky }] = await db.select({ sticky: count() }).from(blogPosts).where(eq(blogPosts.isSticky, true));
...
export async function getBlogAdminAuthors() {
  const db = getDrizzle();
  return db.selectDistinct({ id: user.id, name: user.name }).from(user).innerJoin(blogPosts, eq(blogPosts.authorId, user.id)).orderBy(asc(user.name));
}

export async function getBlogAdminTags(locale: Locale) {
  const db = getDrizzle();

...
/**
 * Recherche plein-texte CMS (pages + blog + services).
 * Couche loader — l'endpoint `api/search.ts` ne fait que transport + rate-limit.
 */
export async function searchContent(q: string, locale: Locale, limit: number): Promise<SearchResultItem[]> {
